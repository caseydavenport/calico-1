...
key `log_level`.  See [Configuring the {{site.prodname}} CNI plugins]({{site.url}}/{{page.version}}/reference/cni-plugin/configuration) for more information.
...
By default, all enabled IP pools are used. However, you can specify which IP pools to use for IP address management in the [CNI network config]({{site.url}}/{{page.version}}/reference/cni-plugin/configuration#ipam),
...
[{{site.prodname}}'s interpretation of Neutron API calls]({{site.url}}/{{page.version}}/networking/openstack/neutron-api)
